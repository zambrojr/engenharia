...
/* harmony default export */ var splitpanesvue_type_script_lang_js_ = ({
  ...
  methods: {
    bindEvents: function bindEvents() {
      var hasTouch = 'ontouchstart' in window; // Passive: false to prevent scrolling while touch dragging.
    ...
    calculatePanesSize: function calculatePanesSize(drag) {
      ...
      var panesToResize = [splitterIndex, splitterIndex + 1]; // When pushOtherPanes = true, find the closest expanded pane on each side of the splitter.
    ...
    doPushOtherPanes: function doPushOtherPanes(sums, dragPercentage) {
      ...
      if (dragPercentage > 100 - sums.nextPanesSize - this.panes[panesToResize[1]].min) {
        ...
        if (panesToResize[1] === undefined) {
          ...
          this.panes.forEach(function (pane, i) {
            if (i < _this.panes.length - 1 && i >= splitterIndex + 1) {
              pane.width = pane.min;
              sums.nextReachedMinPanes += pane.min;
            }
          });
  ...
  render: function render(createEl) {
    ...
    if (!this.$slots.default) splitPanesChildren.push(createEl('div', 'Splitpanes needs some content here.'));else {
      ...
      if (this.watchSlots) {
        var discardProps = ['$options', '$parent', '$root', '$el', '$refs', '$slots', '$scopedSlots', '$vnode', '_data', '__vue__', '_self', '_vnode', '_watcher', '_watchers', '_computedWatchers', '_renderProxy', 'vnodes', 'container', 'Ctor', 'context', 'parent', 'componentInstance', 'componentOptions'];
      ...
      if (this.slotsCount !== this.$slots.default.length || slotsHaveChanged) {
        ...
        this.vnodes.forEach(function (vnode, i) {
          ...
          var _attrs$splitpanesMin = attrs['splitpanes-min'],
              ...
              _attrs$splitpanesDef = attrs['splitpanes-default'],
              Default = _attrs$splitpanesDef === void 0 ? _this2.defaultWidth : _attrs$splitpanesDef;

          _this2.$set(_this2.panes, i, {
            width: parseInt(Default),
            index: i,
            min: parseFloat(min),
            max: parseFloat(max)
          });

          if (i) _this2.$set(_this2.splitters, i - 1, {
      ...
      this.vnodes.forEach(function (vnode, i) {
        ...
        if (i) {
          ...
          var splitterAttributes = {
            ...
            on: (_on = {}, _defineProperty(_on, 'ontouchstart' in window ? 'touchstart' : 'mousedown', function (e) {
              return _this2.onMouseDown(e, i - 1);
            }), _defineProperty(_on, "dblclick", function dblclick(e) {
              return _this2.onSplitterDblClick(e, i);
            }), _on)
          };
        ...
        var paneAttributes = {
          ...
          style: _objectSpread({}, _this2.horizontal ? {
            height: "".concat(_this2.panes[i].width, "%")
          } : null, !_this2.horizontal ? {
            width: "".concat(_this2.panes[i].width, "%")
          } : null),
          on: {
            click: function click(e) {
              return _this2.$emit('pane-click', _this2.panes[i]);
            }
          }
        };
